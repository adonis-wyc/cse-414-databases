...
public class HW6 {
  ...
  public static Dataset<Row> Q1 (SparkSession spark, String dataFile) {
    ...
    df.createOrReplaceTempView("flights");
    Dataset<Row> distictFlightNames = spark.sql("select distinct destcityname from flights where origincityname = 'Seattle, WA'");
    return distictFlightNames;
  }
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
        ...
        d = d.filter(f1 -> f1.get(ORIGIN_CITY_NAME).equals("Seattle, WA"))
       .map(f1 -> RowFactory.create(f1.get(DEST_CITY_NAME)))
       .distinct();
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
          ...
          JavaPairRDD<Tuple2<String, Integer>, Integer> dfn = d.filter(f1 -> f1.get(CANCELLED).equals(0))
        .mapToPair(f1 -> new Tuple2<Tuple2<String,Integer>, Integer>(new Tuple2<String,Integer> (f1.get(ORIGIN_CITY_NAME).toString(), Integer.valueOf(f1.get(MONTH).toString())), 1));
      
      dfn = dfn.reduceByKey((v1,v2) -> v1 + v2);
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Tuple2<String,Integer>> f1 = d.mapToPair(tup -> new Tuple2<Tuple2<String,String>, Integer> (new Tuple2<String,String> (tup.get(ORIGIN_CITY_NAME).toString(),tup.get(DEST_CITY_NAME).toString()), 1))
      .reduceByKey((v1,v2) -> v1 + v2)
      .map(tup -> new Tuple2<String, Integer>(tup._1._1, tup._2));
      
      Tuple2<String,Integer> tup = f1.max(new compareTups());
      return tup;
  }

  private static class compareTups implements Serializable, Comparator<Tuple2<String,Integer>> {
        @Override
        public int compare(Tuple2<String, Integer> tup1, Tuple2<String, Integer> tup2) {
            return Integer.compare(tup1._2, tup2._2);
        }
    }
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
      ...
      JavaRDD<Row> nonNullDelay = d.filter(tup -> tup.get(DEP_DELAY) != null);
      
    JavaPairRDD<String,Double> totalN = nonNullDelay
      .mapToPair(tup -> new Tuple2<String,Double> (tup.get(ORIGIN_CITY_NAME).toString(), Double.valueOf(tup.get(DEP_DELAY).toString())))
      .reduceByKey((v1,v2) -> v1 + v2);
     
      JavaPairRDD<String,Double> countN = nonNullDelay
      .mapToPair(t -> new Tuple2<String,Double> (t.get(ORIGIN_CITY_NAME).toString(), 1.0))
      .reduceByKey((v1,v2) -> v1 + v2);
      
      JavaPairRDD<String, Tuple2<Double,Double>> sumCounts = totalN.join(countN);
      
      JavaPairRDD<String, Double> avg = sumCounts.mapToPair(averageVals);
      
    return avg;
  }

  private static PairFunction<Tuple2<String, Tuple2<Double, Double>>,String,Double> averageVals = (tuple) -> {
